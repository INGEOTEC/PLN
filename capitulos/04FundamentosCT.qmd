...
class ClasificadorMNGram:
  ...
  def fit(self, X, y):
    if self.modelos is not None:
      return self
    self.uniq_labels, priors = np.unique(y, return_counts=True)
    self.priors = np.log(priors) - np.log(priors.sum())
    modelos = []
    for clase in self.uniq_labels:
      _ = ModeloNGram(N=self.N,
                      k_suavizado=self.k_suavizado).fit(
        [x for x, _y in zip(X, y) if _y == clase]
      )
      modelos.append(_)
  ...
  def predict_log_proba(self, X):
    X = np.column_stack(
      [m.log_prob(X) for m in cl.modelos]
    )
