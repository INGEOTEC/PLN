...
for l in length.rvs(size=1000): # <6>
    D.append((id2w(pos.rvs(round(l))), 1)) # <7>
    D.append((id2w(neg.rvs(round(l))), 0)) # <8>
```
...
El primer paso es encontrar la verosimilitud dado el conjunto de datos `D`. El siguiente código calcula la verosimilitud de la clase positiva asumiendo que todas las palabras, (i.e., *w*, *x*, *y* y *z*) son independientes. 
...
for data, k in D: # <1>
    if k != 1: # <2>
        continue 
    segmentos = data.split() # <3>
    D_pos.extend(segmentos)
words, l_pos = np.unique(D_pos, return_counts=True) # <4>
w2id = {v: k for k, v in enumerate(words)} # <5>
l_pos = l_pos / l_pos.sum() # <6>
pp(l_pos)
```

1. Ciclo por los elementos del conjunto de entrenamiento
2. Analizar solos los elementos asociados a la clase positiva
3. Segmentar el texto 
4. Calcular la frecuencia de cada segmento
5. Función para identificar el índice de cada palabra
6. Probabilidad de cada segmento en la clase positiva
