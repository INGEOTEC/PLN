...
Para el modelo de lenguaje se requiere calcular la frecuencia de la secuencia (variable `sentence`), i.e., $\#(w_{n-N+1}, \ldots, w_{n-1}, w_n)$ y la frecuencia de las palabras previas (variable `history`), i.e., $\#(w_{n-N+1}, \ldots, w_{n-1})$, dependiendo de $N$. Adicionalmente, se guarda el vocabulario en la variable `voc`.  
...
pp([bilmaLAT.decode(list(map(int, k.split("|"))))
    for k, v in sentence.most_common(10)])
```
...
#| echo: true
pp([bilmaLAT.decode(list(map(int, k.split("|"))))
    for k, v in history.most_common(10)])
```
