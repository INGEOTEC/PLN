...
Un modelo de lenguaje es una función que permite estimar la probabilidad de un texto, i.e., $\mathbb P(w_1, w_2, w_n).$ Comunmente, se trata de modelar  $\mathbb P(w_n \mid w_{n-1}, w_{n-2}, \ldots, w_1),$ es decir, encontrar una función que estime la probabilidad de la siguiente palabra o segmento, i.e., $w_N$, dada la historia. 

Realizando una suposición de Markov, es decir, que el segmento $w_n$, es independiente de los segmentos iguales y mayores a $N$, es decir, $\mathbb P(w_n \mid w_{n-1}, w_{n-2}, \ldots, w_1)=\mathbb P(w_n \mid w_{n-1}, w_{n-2}, \ldots, w_{n-N+1}).$

La segunda parte de esta libreta ejemplifica la creación de modelo de lenguaje utilizando gramas y además hace una suposición de Markov.

El siguiente código asume un modelo de bigramas ($N=2$), es decir, la probabilidad de la siguiente palabra solamente depende de la palabra previa.
...
Para el modelo de lenguaje se requiere calcular la frecuencia de la secuencia (variable `sentence`) y la frecuencia de las palabras previas (variable `history`), dependiendo de $N$. Adicionalmente, se guarda el vocabulario en la variable `voc`.  
...
```{python}
#| echo: true
[bilmaLAT.decode(list(map(int, k.split("|")))) for k, v in sentence.most_common(10)]
```
...
```{python}
#| echo: true
[bilmaLAT.decode(list(map(int, k.split("|")))) for k, v in history.most_common(10)]
```
...
```{python}
#| echo: true
sentence["|".join(map(str, code))] / history["|".join(map(str, code[:-1]))]
```
...
def prob(txt, k=0.01):
  code = bilmaLAT(txt)['input_ids'][1:-1]
  num = sentence["|".join(map(str, code))] + k
  den = history["|".join(map(str, code[:-1]))] + k * len(voc)
  return num / den
```
...
def log_prob(ele, k=1, average=True):
  ...
  for code in zip(*[tokens[i:] for i in range(N)]):
    num = np.log(sentence["|".join(map(str, code))] + k)
    den = np.log(history["|".join(map(str, code[:-1]))] + k * len(voc))
    acc += num - den
  if average:
