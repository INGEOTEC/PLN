...
Habiendo definido las palabras especiales, es momento de inicializar la clase que se usa para segmentar, esta clase es `Tokenizer`, la cual requiere el método que encuentra en los segmentos, en esta ocasión se utiliza el método `WordPiece` que requiere solamente conocer cual es el identificador de los segmentos desconocidos (ver primera linea). En la segunda linea se añaden los identificadores especiales al segmentador. 

```{python}
#| echo: true
#| output: false

tokenizer = Tokenizer(WordPiece(unk_token=special_tokens[0].content))
