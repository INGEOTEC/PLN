...
from CompStats.metrics import f1_score, macro_f1,\
                              recall_score,\
                              precision_score
...
CompStats.utils.USE_TQDM = False
```
...
a = f1_score(y, hy_bow, average=None).statistic
...
#| echo: true
y = np.r_[[x['klass'] for x in Dtest]]
f1_score(y, hy_bow, average=None).statistic
```
...
```{python}
#| echo: true
score = f1_score(y, hy_bow,
                 average=None,
                 name='BoW')
ci_izq, ci_der = score.ci
```
...
df_bow = score.dataframe()
sns.displot(df_bow, x='Score', 
            col='Performance', kde=True)
```
...
#| echo: true
#| output: false
_macro_f1 = lambda y, hy: macro_f1(y, hy).statistic
...
El rendimiento en $f_1$ del clasificador basado en una bolsa de palabras densa se calcula con el siguiente código. Este valor puntual se complementa con la @fig-hist-f1-bow-dense donde se muestra el rendimiento entre las dos bolsas de palabras.

```{python}
#| echo: true
score(hy_dense, name='Densa')
score.statistic['Densa']
```
...
score.plot(var_name='Rendimiento', alg_legend='Algoritmo',
...
//| label: fig-prediccion-1-estimador
//| file: fig/stack_generalization.dot
```
...
f1_score(y, hy_stack, average=None).statistic
```
...
recall_bow = recall_score(y, hy_bow, average=None).statistic
recall_dense = recall_score(y, hy_dense, average=None).statistic
recall_stack = recall_score(y, hy_stack, average=None).statistic
pr_bow = precision_score(y, hy_bow, average=None).statistic
pr_dense = precision_score(y, hy_dense, average=None).statistic
pr_stack = precision_score(y, hy_stack, average=None).statistic
...
f1_bow =   f1_score(y, hy_bow, average=None).statistic
f1_dense = f1_score(y, hy_dense, average=None).statistic
f1_stack = f1_score(y, hy_stack, average=None).statistic

f1_macro_bow =   f1_score(y, hy_bow,   average="macro").statistic
f1_macro_dense = f1_score(y, hy_dense, average="macro").statistic
f1_macro_stack = f1_score(y, hy_stack, average="macro").statistic
