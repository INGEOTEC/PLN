...
Lo primer que se realiza es cargar el conjunto de datos que ha sido utilizado en el @sec-fund-cl-texto y se crea un modelo de lenguaje (i.e., `ModeloNGram`) de unigramas y sin $k$-suavizado. 
...
@pennington-etal-2014-glove @mikolov2013efficientestimationwordrepresentations
...
El método `similarity` calcula la similitud coseno entre las dos representaciones. La representación vectorial se encuentra en el atributo `vector`; el atributo `vector_norm` contiene la norma y estos dos atributos pueden ser utilizados para calcular la similitud coseno entre las dos palabras utilizadas anteriormente, como se puede observar en la siguiente instrucción.
...
casa_hogar_man = dense.transform(
  ['casa', 'hogar', 'manzana']
)
```

`{python} Markdown(f'{np.dot(X[0], X[1]):0.4f}')`

```{python}
#| echo: true

manzana = pln('manzana')[0]
casa.similarity(manzana)
```

`{python} Markdown(f'{np.dot(X[0], X[2]):0.4f}')`
...
En la siguiente instrucción es modela el segundo elemento del conjunto `dataset`.
...
Las variables `ele1` y `ele2` tienen la representación semántica del primer y segundo texto del conjunto `dataset` se puede observar, en la siguiente instrucción, que estas representaciones se representan como una matriz de 10 y 11 renglones en $\mathbb{R}^{300}.$
